...
// cylinder candidates
ros::Publisher publish_cylinder_candidates;
visualization_msgs::MarkerArray cylinder_candidates_array;
...
cloud_cb (const pcl::PCLPointCloud2ConstPtr& cloud_blob)
{
  ...
  std::cerr << "DOBIM POINT CLOUD" << std::endl;
  ...
  else
  {
	 // std::cerr << "PointCloud representing the cylindrical component: " << cloud_cylinder->points.size () << " data points." << std::endl;
          ...
          catch (tf2::TransformException &ex)
	  {
          ...
          if (std::isnan(point_map.point.x) || (point_map.point.z > 0.5)) {
            return;
          }
        ...
        cylinder_candidates_array.markers.push_back(marker);
        publish_cylinder_candidates.publish(cylinder_candidates_array);

        std::cerr << "DOLZINA: "<<  cylinder_candidates_array.markers.size() << std::endl;
        for (int i = 0; i < cylinder_candidates_array.markers.size(); i++) {
          std::cerr << "X: " <<  cylinder_candidates_array.markers[i].pose.position.x << " Y: " <<  cylinder_candidates_array.markers[i].pose.position.x  << std::endl;
        }
